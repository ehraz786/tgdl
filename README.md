...
- **Advanced YouTube Downloads**: When you send a YouTube link with `/ytupload`, the bot now provides three download options:
  ...
  - **Important**:`/ytupload`,Now not using Regular,zip,unzip... buttons. It deafult set to Regular uploads (large files upto 2gb automatically get in to ziped).If you want to use Regular,unzip,zip.. buttons kindly use `/tupload` for ytdl link.As well as you can use video spilit function using `/tuploads`.
